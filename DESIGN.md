...
Requests are authenticated by `backend/src/middleware/auth.ts`, which reads `x-user-id` and attaches the seeded user to `req.user`. Read access is checked by `canViewDocument()` in `backend/src/routes/documents.ts`; owners always see their own documents, viewers only see published documents, reviewers do not see drafts or archived documents they do not own, and admins can view archived documents through the admin-only `includeArchived` path. Mutation permissions are checked both in route guards and in `transition.ts`'s validators; document creation, submit, and reopen are author-only, while archive remains admin-only.
